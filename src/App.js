...
class App extends Component {
  ...
  validate = ({ urssafCode, year, siret }) => {
    const errors = [];
    ...
    if (siret.length !== 14) {
      errors.push(['siret', `Le code SIRET doit faire 14 caratères, il en fait ${siret.length}.`])
    }

    return errors.length ? errors.reduce((errs, [key, val]) => ({ ...errs, [key]: val }), {}) : undefined;
  };

  decidePeriodType(periodType) {
    this.setState({
      page: 'codeParts',
      periodType,
      codeParts: calculateInitialCodeParts(periodType),
    });
  }

  calculateCode = (codeParts) => {
    const code = `${codeParts.urssafCode}${codeParts.year}${codeParts.periodCode}${codeParts.siret}`;
    if (code.length !== 24) {
      return { length: `Le code devrait faire 24 caractères, il en fait ${code.length}.` };
    }
    localStorage.setItem('urssafCode', codeParts.urssafCode);
    localStorage.setItem('siret', codeParts.siret);

    this.setState({ page: 'code', codeParts, code })
  };

  render() {
    const { page, codeParts, code, periodType } = this.state;

    if (page === 'periodType') {
      return (
        <div className="App">
          <h1>Type de périodicité</h1>
          <button onClick={() => this.decidePeriodType('trimester')}>Trimestrielle</button>
          <button onClick={() => this.decidePeriodType('month')}>Mensuelle</button>
          <button onClick={() => this.decidePeriodType('regularization')}>Régularisation annuelle</button>
        </div>
      );
    }
    ...
    if (page === 'code') {
      return (
        <div className="App">
          <h1>
            <button onClick={() => this.setState({ page: 'codeParts' })}>&lt;</button>
            Code
          </h1>
          <p>{code}</p>
        </div>
    ...
    return (
      <div className="App">
        <h1>
          <button onClick={() => this.setState({ page: 'periodType' })}>&lt;</button>
          Périodicité:
        ...
        <Form
          ...
          onSubmit={this.calculateCode}
          render={({ handleSubmit }) => (
            <form onSubmit={handleSubmit}>
              <div>
                <label for='urssafCode'>Code organisme URSSAF</label>
                <Field
                  name='urssafCode'
                  component='input'
                />
              ...
              <div>
                <label for='periodCode'>Période</label>
                {periodType === 'trimester' && (
                  <Field
                    name='periodCode'
                    component='select'
